...
/*   Updated: 2024/05/29 14:50:41 by wlucke           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (read(fd, &buffer[fd], 0) < 0)
		return (NULL);
	line = ft_strdup((const char *) buffer[fd] + 1);
	nbytes = 1;
	while (check_new_line(line) == 0 && nbytes != 0)
	{
		ft_bzero(buffer[fd], BUFFER_SIZE);
		nbytes = read(fd, buffer[fd], BUFFER_SIZE);
		tmp = ft_strjoin(line, (char const *)buffer[fd]);
		free(line);
		line = tmp;
	}
	line = clean_save_cut(buffer[fd], line, nbytes);
	return (line);
}
